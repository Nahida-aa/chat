...
export function initIo(httpServer: HttpServer) {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("disconnect", (reason) => {
      const userId = socket.data.userId;
      console.log(
        `ws.disconnect ${socket.id} user: ${userId} reason: ${reason}`,
  ...
  instrument(io, {
    auth: {
      type: "basic",
      username: "admin",
      password: process.env.SOCKET_IO_ADMIN_PASSWORD_HASH!, // https://socket.io/docs/v4/admin-ui/#available-options
