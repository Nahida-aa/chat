...
import { fileGroup } from "@/features/upload/schema/constants";
import type { file } from "@/features/upload/schema/tables";
...
>; // key: file.name
export const _newFileUploadProgress = <Extra>(
  file: File,
): _FileUploadProgress<Extra> =>
  ({
    file,
...
export const signedUrlRes = z.object({
  ...
  storageKey: z
    .string()
    .meta({ example: "user/{authId}/{group}/{fileId}/{name}" }),
});
